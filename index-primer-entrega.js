...
// Aquí declaramos la función COMPRAR FIXIE, donde se encuentra la seleccion de Modelo y Cantidad de las mismas.
//Esta función tambíen tiene un parámetro (num1) con la cual le ponemos un orden a las compras que va realizando.
function comprarFixie(num1) {
    // aqui declaramos la funcion para seleccionar el MODELO.
    const selecionarModelo = () => {
        const Classic = "a";
        const Jaguar = "b";
        const Bullmoose = "c";
        let modeloLower = prompt("¿Qué modelo de fixie desea? Seleccione el numero \n Selecciona la letra que corresponda: \n A - Classic - $110000 IVA no incluido. \n B - Jaguar - $125000 IVA no incluido. \n C - Bullmoose - $13500 IVA no incluido.");
        let modelofix = modeloLower.toLocaleLowerCase();
    ...
    const selecionarColor = () => {
        let colorLower = prompt(`${modelofix}\n¿Cuál color deseas?\nColores disponibles: rojo, azul, negro`);
        let color = colorLower.toLocaleLowerCase();
        if ((color === colorrojo) || (color === colorazul) || (color === colornegro)) {
            return color;
        } else {
...
function bucleDeCompra() {
    for (let i = 1; ((i <= 4) && (comprar == true)); i++) {
        //Aquí llamamos a nuestra función comprarFixie donde "i" es el parámetro.
